...
export default function Empleado({ user, token, onLogout, toast, dark, toggleDark, fichajeHoy, pausas, pausaActiva, onRefreshFichaje, onSetFichajeHoy, onSetPausas, onSetPausaActiva }) {
  ...
  async function ficharSalida() {
    ...
    try {
      // Si hay pausa activa, la cerramos antes
  ...
  const props = { user, token, toast, dark, toggleDark, onLogout, checkedIn, fichajeHoy, pausas, pausaActiva, netMs, now, onFicharEntrada: ficharEntrada, onFicharSalida: ficharSalida, onPausar: pausar, onReanudar: reanudar, onRefresh: onRefreshFichaje }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <aside className="desktop-sidebar" style={{ width: 200, background: 'var(--surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          ...
          <div style={{ padding: '18px 16px', borderBottom: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: 9, padding: '1px 6px', borderRadius: 8, background: checkedIn ? 'rgba(143,184,160,0.2)' : 'rgba(128,128,128,0.1)', color: checkedIn ? 'var(--accent2)' : 'var(--muted)', border: `1px solid ${checkedIn ? 'var(--accent2)' : 'var(--border)'}` }}>
                    {checkedIn ? (pausaActiva ? 'En pausa' : 'En oficina') : 'Fuera'}
                  </span>
                </div>
              </div>
              <button onClick={toggleDark} style={{ background: 'transparent', border: 'none', color: 'var(--muted)', fontSize: 14, cursor: 'pointer', padding: 2 }}>{dark ? '☀' : '☾'}</button>
            </div>
            ...
            <div style={{ background: 'var(--surface2)', borderRadius: 6, padding: '8px 10px', marginBottom: 8, border: `1px solid ${pausaActiva ? 'var(--accent)' : checkedIn ? 'var(--accent2)' : 'var(--border)'}` }}>
              <div style={{ fontSize: 9, letterSpacing: 2, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 2 }}>
                {pausaActiva ? `⏸ En pausa (${pausaActiva.tipo})` : checkedIn ? '▶ Tiempo trabajado' : 'Sin entrada activa'}
              </div>
              <div style={{ fontSize: 16, fontWeight: 'bold', fontVariantNumeric: 'tabular-nums', color: pausaActiva ? 'var(--accent)' : checkedIn ? 'var(--accent2)' : 'var(--muted)', letterSpacing: 1 }}>
                {fmtDurS(netMs)}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {checkedIn && !fichajeHoy?.salida && (
                pausaActiva
                  ? <button onClick={reanudar} style={{ flex: 1, background: 'var(--accent)', color: '#0f0f0f', border: 'none', borderRadius: 4, padding: '6px 0', fontSize: 10, fontWeight: 'bold', cursor: 'pointer', letterSpacing: 1 }}>▶ REANUDAR</button>
                  : <button onClick={() => setPausaModal(true)} style={{ flex: 1, background: 'var(--surface2)', color: 'var(--muted)', border: '1px solid var(--border)', borderRadius: 4, padding: '6px 0', fontSize: 10, fontWeight: 'bold', cursor: 'pointer', letterSpacing: 1 }}>⏸ PAUSA</button>
              )}
              {checkedIn && !fichajeHoy?.salida && (
                <button onClick={ficharSalida} style={{ flex: 1, background: 'var(--danger)', color: '#fff', border: 'none', borderRadius: 4, padding: '6px 0', fontSize: 10, fontWeight: 'bold', cursor: 'pointer', letterSpacing: 1 }}>⏏ SALIDA</button>
              )}
              {!checkedIn && !fichajeHoy?.entrada && (
                <button onClick={ficharEntrada} style={{ flex: 1, background: 'var(--accent2)', color: '#0f0f0f', border: 'none', borderRadius: 4, padding: '6px 0', fontSize: 10, fontWeight: 'bold', cursor: 'pointer', letterSpacing: 1 }}>↑ ENTRADA</button>
              )}
            </div>
          </div>
        ...
        <main className="main-pad" style={{ flex: 1, overflow: 'auto', padding: 26, background: 'var(--bg)' }}>
          <TopBar user={user} dark={dark} toggleDark={toggleDark} onLogout={onLogout}
            ...
            onPausar={() => setPausaModal(true)} onReanudar={reanudar} onSalida={ficharSalida}
          />
          {tab === 'fichar'     && <EmpFichar {...props} onShowPausa={() => setPausaModal(true)} />}
      ...
      {/* Modal de tipo de pausa */}
      <PausaModal onConfirm={tipo => { setPausaModal(false); pausar(tipo) }} onCancel={() => setPausaModal(false)} visible={pausaModal} />
    </div>
  )

  function setPausaModal(v) { _setPausaModal(v) }
}

// Necesitamos el useState fuera del render de los botones — lo elevamos al componente
// El truco: usamos una función auxiliar para acceder al estado modal de pausa
// Se declara correctamente abajo mediante el patrón de ref de setter
